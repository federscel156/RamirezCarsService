...
public class OrdenTrabajoDAO {
    ...
    /*
     * Función de Extracción de Interfaces Dinámicas.
     * Recupera exclusivamente las Órdenes de Trabajo en estado activo ('Abierta'),
     * concatenando el identificador y la patente para facilitar la selección en la capa visual (UX).
     */
    public java.util.List<String> obtenerOrdenesAbiertas() {
        java.util.List<String> ordenes = new java.util.ArrayList<>();
        String sql = "SELECT nro_ot, patente FROM Orden_Trabajo WHERE estado = 'Abierta'";
        ...
        try (Connection conn = ConexionBD.conectar();
            ...
            while (rs.next()) {
                // Formato de salida: "123 - Patente: AB123CD"
                ordenes.add(rs.getInt("nro_ot") + " - Patente: " + rs.getString("patente"));
            }
        } catch (SQLException e) {
            System.err.println("Error al recuperar el listado de OTs abiertas: " + e.getMessage());
        }
        return ordenes;
    }
}